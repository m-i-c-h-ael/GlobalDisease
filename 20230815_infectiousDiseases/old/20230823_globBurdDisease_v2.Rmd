...
groupedDF= allTime_HT_asi_D %>%
      group_by(location,cause) %>%
      summarize(n()) 
dim(groupedDF) #42 combinations
head(allTime_HT_asi_D)

if(! file.exists('G:/My Drive/20230815_GlobBurdDisease/20230823_Deaths_HTZ_21reg_MF_5year/DeathRateDown.csv')){ #execute only if not yet done
    ...
    if(HDIupper_b2<0){
      DeathrateDown_DF= rbind.data.frame(DeathrateDown_DF,
              c(as.character(groupedDF$location[i]),as.character(groupedDF$cause[i]),
                median(mcmcDF$b0),median(mcmcDF$b1),median(mcmcDF$b2),
                median(mcmcDF$t0)) )
    }
  }
  colnames(DeathrateDown_DF)= c('location','cause','b0_median','b1_median','b2_median','t_inflec_median')
  
  # write output so you do not need to run this code every time
  write.csv(DeathrateDown_DF,
            'G:/My Drive/20230815_GlobBurdDisease/20230823_Deaths_HTZ_21reg_MF_5year/DeathRateDown.csv',
            row.names = FALSE)
} else {
  DeathrateDown_DF= read.csv(
            'G:/My Drive/20230815_GlobBurdDisease/20230823_Deaths_HTZ_21reg_MF_5year/DeathRateDown.csv')
}
...
```{R}
summary(DeathrateDown_DF[,3:6])
str(DeathrateDown_DF)

#manually sorted
DeathrateDown_DF_sort= DeathrateDown_DF[
  order(DeathrateDown_DF$t_inflec_median,DeathrateDown_DF$b2_median),]
DeathrateDown_DF_sort$location= as.factor(DeathrateDown_DF_sort$location)
DeathrateDown_DF_sort$cause= as.factor(DeathrateDown_DF_sort$cause)

DrD= DeathrateDown_DF_sort[DeathrateDown_DF_sort$cause=='HIV/AIDS',]

DrD_melt= melt(DrD,id.vars='location',measure.vars=c('b0_median','b1_median','b2_median','t_inflec_median'))
str(DrD_melt)

DrD_melt$coefType= rep('rate',dim(DrD_melt)[1])
DrD_melt$coefType[DrD_melt$variable=='t_inflec_median']= 'year'
head(DrD_melt)

#split into two DFs
# DrD_rates= DrD_melt[DrD_melt$coefType=='rate',]
# DrD_rates$variable= factor(DrD_rates$variable,
#         levels=c('b0_median','b1_median','b2_median'))
# DrD_year= DrD_melt[DrD_melt$coefType=='year',]
# DrD_year$variable= factor(DrD_year$variable,levels='year')

ggplot()+
  geom_tile(data=DrD_melt[DrD_melt$coefType=='rate',],aes(x=variable,y=location,fill=value))+
  scale_fill_distiller(type = "seq", palette = "Greens", guide = guide_colorbar(title.position = "top")) +
  ggnewscale::new_scale_fill() +
  geom_tile(data=DrD_melt[DrD_melt$coefType=='year',],aes(x=variable,y=location,fill=value))+
...
heatmap.2(as.matrix(DeathrateDown_DF_sort[,3:6]),
          scale='none', #'column',  #
          breads= seq(1990,2019,5),
          Colv=FALSE, Rowv=FALSE,    #no re-ordering
          dendrogram='none', #'row', 
          labRow=DeathrateDown_DF$location, 
          RowSideColors = c('red','blue')[as.factor(DeathrateDown_DF$cause)],
          cexCol=1,srtCol=0)


#automatically sorted
heatmap.2(as.matrix(DeathrateDown_DF[,3:6]),
          scale='column', #'none',
          Colv=FALSE,
          dendrogram='row', 
          labRow=DeathrateDown_DF$location, 
