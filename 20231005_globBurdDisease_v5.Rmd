...
* Hierarchical model (e.g. countries within continent) - also for ML
* Analyse outliers
* Prevalence vs incidence vs death
* Adjust normalization to population (e.g. within age groups)
...
Deaths_file= './20230823_Deaths_HTZ_21reg_MF_5year/IHME-GBD_2019_DATA-afd4b08e-1.csv'  #real data
Deaths_file= './testData/test1.csv'                                                    #test

if( Deaths_file== './20230823_Deaths_HTZ_21reg_MF_5year/IHME-GBD_2019_DATA-afd4b08e-1.csv' ) {
...
} else if (Deaths_file== './testData/test1.csv' ){
  allTime_HTZ= read.csv('./testData/test1.csv',stringsAsFactors = TRUE)
}
...
for (ca in unique(allTime_HTZ$cause)){
  ...
  if(len != 3){   #Incidence, Prevalence, Deaths
...
if( (Deaths_file== './20230823_Deaths_HTZ_21reg_MF_5year/IHME-GBD_2019_DATA-afd4b08e-1.csv' &  #real data
      ! file.exists('./20230914_MCMCcoefs.csv')) |
    (Deaths_file== './testData/test1.csv' &   #test
      ! file.exists('./test1_MCMCcoefs.csv')) )  {  #execute only if not yet done
  ...
  for(i in 1:dim(groupedDF)[1]){
    ...
    if( iF2_best==0){  #if there is no decrease, there is no inflection point
      mcmcDF$t0= NA
    } else if (iF1_best==0) { #if there has never been increase, set inflection point to first TP
      mcmcDF$t0= 1990
    }
  ...
  if(Deaths_file== './20230823_Deaths_HTZ_21reg_MF_5year/IHME-GBD_2019_DATA-afd4b08e-1.csv'){
    ...
    write.csv(MCMCcoefs_DF, './test1_MCMCcoefs.csv', row.names = FALSE)
  }
} else {
  if(Deaths_file== './20230823_Deaths_HTZ_21reg_MF_5year/IHME-GBD_2019_DATA-afd4b08e-1.csv'){
    MCMCcoefs_DF= read.csv('./20230914_MCMCcoefs.csv')
  } else if(Deaths_file== './testData/test1.csv') {
    MCMCcoefs_DF= read.csv('./test1_MCMCcoefs.csv')
  }
}
str(MCMCcoefs_DF)
dim(MCMCcoefs_DF)
...
hist(MCMCcoefs_DF$b2_median[MCMCcoefs_DF$b2_median<0],breaks=50,main='Settings with decr. death rate',
     xlab='% / year')
if( !all(is.na(MCMCcoefs_DF$t_inflec_median))){
  hist(MCMCcoefs_DF$t_inflec_median,breaks=50,main='Inflection year', xlab='year')
} else {
...
t= unique(allTime_HT_asi$year)
predDF= data.frame(matrix(NA,nrow=dim(MCMCcoefs_DF)[1],ncol=length(t)))
for(i in 1:dim(MCMCcoefs_DF)[1]){
  if(is.na(MCMCcoefs_DF$t_inflec_median[i])){
    t_pastInv= rep(0,length(t))
  } else {
    t_pastInv= t - MCMCcoefs_DF$t_inflec_median[i]
    t_pastInv[t_pastInv<0]= 0
  }
  predDF[i,]= MCMCcoefs_DF$b0_median[i] + MCMCcoefs_DF$b1_median[i]*(t-1990) +MCMCcoefs_DF$b2_median[i]* t_pastInv
}
colnames(predDF)= t
predDF= cbind.data.frame(location=MCMCcoefs_DF$location, cause=MCMCcoefs_DF$cause, measure=MCMCcoefs_DF$measure, predDF)
predDF_melt= melt(data=predDF,id.vars=c('location','cause','measure'), variable.name='year')
predDF_melt$year= as.numeric(as.character(predDF_melt$year))

ggplot()+
  geom_point(data=allTime_HT_asi[ allTime_HT_asi$cause=='HIV/AIDS',  ],
         mapping=aes(x=year,y=val,group=measure,color=measure))+
  geom_line(data=predDF_melt[ predDF_melt$cause=='HIV/AIDS',  ],
         mapping=aes(x=year,y=value,group=measure,color=measure))+
  ggtitle('HIV/AIDS')+
  guides(color="none")+
...
ggplot()+
  geom_point(data=allTime_HT_asi[ allTime_HT_asi$cause=='Tuberculosis',  ],
         mapping=aes(x=year,y=val,group=measure,color=measure))+
  geom_line(data=predDF_melt[ predDF_melt$cause=='Tuberculosis',  ],
         mapping=aes(x=year,y=value,group=measure,color=measure))+
...
It should be possible to construct the following model for prevalence, based on incidence, prevalence and deaths. Use absolute number, not percentages for 'D' and 'H' to also include changes in population size.
![](20230921_epidemRatesModel_1.jpg)
![](20230921_epidemRatesModel_2.jpg)
For a complete model, you will need information on individuals w/o disease (number of people and deaths) and cure-rate. If these additional data can be obtained, you could include an error term on the prevalence to estimate how far off these values likely are.
...
#Decision trees / Random forest probably does not need one-hot encoding, but probably still numeric values
????????????
...
?????????
MCMCcoefs_DF_HTsex_age['sex']= MCMCcoefs_DF_HTsex_age['sex'].astype('int')
```
```{python}
